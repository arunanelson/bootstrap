...
(function($) {
	...
	$.fn.iSuggest = function(settings) {
		...
		settings = jQuery.extend({
			url: '../inc/results/mysql_suggestions_results.php', // Suggestions results script.
			...
			searchURL: 'http://localhost:8080/bormiolirocco/bootstrap/products/line?id=%s' // Set search query URL. URL with %s in place of query.
		}, settings);
		...
		return $(this).each(function() {
			...
			container.find('ul li a').live('mouseover', function(){
					container.find('ul li a').removeClass('active');
			});

			// Set select callback
			if($.isFunction(settings.onSelect)) container.find('ul li a').live('click', function(){
				el.val($(this).text());
